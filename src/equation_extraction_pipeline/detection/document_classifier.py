"""Document classifier — page/document modality and category classification.

Extracted from ``equation_label_detector.py`` (its former Sections 1-6). This
module owns the *document classification* responsibility only; equation layout
detection remains in ``equation_label_detector.py``. The two concerns share no
symbols.
...
def collect_signals(
    ...
    *,
    ...
    signals.figure_density = sum(image_coverages) / len(image_coverages) if image_coverages else 0.0
...
def determine_modality(
    ...
    *,
    ...
    proportions = {
        ptype: round(count / total, 6) for ptype, count in sorted(page_type_counts.items())
    }
...
def _category_raw_score(category: str, signals: DocumentSignals) -> float:
    ...
    title = (signals.metadata.get("Title", "") + " " + signals.metadata.get("title", "")).lower()
...
def score_categories(signals: DocumentSignals, taxonomy: list[str]) -> list[CategoryCandidate]:
...
def _taxonomy() -> list[str]:
    return [c.strip() for c in config.CLASSIFIER_DOC_CATEGORIES.split(",") if c.strip()]
...
def classify_document(
    ...
    *,
    ...
    try:
        ...
        _structlog.error("document_classification_failed", pdf=str(pdf_path), error=str(exc))
        strategy = recommend_strategy(
            modality="scanned",
            category="unknown",
